...
1. **Base 2 vs Base 10:**
   - نکته => `double`: Base 2 → نمی‌تواند اعداد اعشاری Base 10 را دقیق نمایش دهد
   - نکته => `decimal`: Base 10 → می‌تواند اعداد اعشاری Base 10 را دقیق نمایش دهد
...
- نکته => Runtime و پردازنده به‌صورت کارآمد با حداقل 1 بایت کار می‌کنند
...
```
**قانون:** عملگرهای مقایسه روی `enum` بر اساس **مقدار Integral ** کار می‌کنند (جزئیات در صفحه 154).
...
### Implicit Conversion
 نکته =>`char` به صورت Implicit به انواعی که **unsigned short** (2 بایت بدون علامت) را پشتیبانی می‌کنند، تبدیل می‌شود.
...
- نکته => `char` یک **Value Type** عددی است (unsigned 16-bit)
...
- نکته => C# به صورت خودکار `ToString()` را فراخوانی می‌کند
...
- نکته => String Interpolation خوانایی کد را افزایش می‌دهد
...
- نکته => JIT می‌تواند قبل از ورود به حلقه ایمنی را تشخیص دهد
- در این صورت بررسی در هر iteration حذف می‌شود
- هزینه performance معمولاً ناچیز است

**گزینه Unsafe:**

- نکته => C# امکان bypass کردن bounds checking را دارد
...
2.  نکته =>Reference به شیء برگردانده می‌شود
3.  نکته =>Garbage Collector → Deallocation اشیاء بلااستفاده
...
- خود شیء → همیشه Heap
- نکته => Reference به شیء → بستگی به محل تعریف
...
- نکته => GC به صورت خودکار اشیاء بلااستفاده را حذف می‌کند
...
- نکته => Garbage Collector
- نکته => Static fields (تا پایان process)

**Bounds Checking:**
- امنیت + سادگی debugging
- نکته => JIT optimization
- هزینه performance ناچیز

**قانون طلایی:**

 نکته =>Reference به شیء → محل تعریف متغیر
خود شیء → همیشه Heap
...
- نکته => **Value (بدون modifier):** کپی مقدار ارسال می‌شود
-  نکته =>**ref:** ارجاع ارسال می‌شود، متغیر باید قبل از فراخوانی مقداردهی شده باشد
-  نکته =>**in:** ارجاع فقط‌خواندنی، متغیر باید قبل از فراخوانی مقداردهی شده باشد
- نکته => **out:** ارجاع ارسال می‌شود، متد باید قبل از بازگشت مقداردهی کند
...
```
**توضیح:**
-  نکته => `sb` و `fooSB` → دو متغیر جداگانه
- هر دو به **یک شیء** اشاره دارند
- نکته => `fooSB.Append()` → شیء اصلی را تغییر می‌دهد
-  نکته => `fooSB = null` → فقط کپی محلی را null می‌کند، `sb` همچنان معتبر است
...
```
**الزامات:**
- نکته => `ref` هم در تعریف متد و هم در فراخوانی لازم است
